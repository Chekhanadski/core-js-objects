...
function makeWord(lettersObject) {
  ...
  values.forEach((indxs, i) =>
    indxs.forEach((indx) => {
      lettersArray[indx] = keys[i];
    })
  );
...
function sellTickets(queue) {
  ...
  const billsObject = { 25: 0, 50: 0, 100: 0 };
  if (queue[0] !== 25) {
    return false;
  }
  return queue.every((bill) => {
    switch (bill) {
      case 25:
        ...
        break;
      case 50:
        if (billsObject[25] === 0) return false;
        billsObject[25] -= 1;
        billsObject[50] += 1;
        break;
      case 100:
        if (billsObject[50] > 0 && billsObject[25] > 0) {
          billsObject[50] -= 1;
          billsObject[25] -= 1;
        } else if (billsObject[25] >= 3) {
          billsObject[25] -= 3;
        } else {
          return false;
        }
...
class CssSelector {
  ...
  element(value) {
    this.checkOrder(this.order.element);
    this.checkCount('element');
    this.selector += value;
    return this;
  }

  id(value) {
    this.checkOrder(this.order.id);
    this.checkCount('id');
    this.selector += `#${value}`;
    return this;
  }
  ...
  class(value) {
    this.checkOrder(this.order.class);
    this.selector += `.${value}`;
    return this;
  }

  attr(value) {
    this.checkOrder(this.order.attr);
    this.selector += `[${value}]`;
    return this;
  }

  pseudoClass(value) {
    this.checkOrder(this.order.pseudoClass);
    this.selector += `:${value}`;
    return this;
  }

  pseudoElement(value) {
    this.checkOrder(this.order.pseudoElement);
    this.checkCount('pseudoElement');
    this.selector += `::${value}`;
    return this;
  }
